...
echo -e "\033[1;32m Select environment From Where You Want To Export PRD Dump: 
		...
		#press 3 for PERF-ENV Export PRD Dump
        #press 4 for COLLECTCM Export PRD Dump                                   
        #press 5 for INTCM Export PRD Dump                                        
...
flag=1
chmod 777 IIMSPRD_$timestamp.dmp
#adding new code for the dump extract for PERF env 
#=======================================

elif [ $variable1 -eq 3 ] 
then
echo -e "\033[1;34m>>>>>>>>>>>>>>>>> Exporting PRD Dump File from COLLECTCM ..........................\033[0m"
expdp system/IIMS@PERFDBENV directory=DUMP_bkup dumpfile=IIMSPRD_$timestamp.dmp logfile=IIMSPRD_Export_$timestamp.log INCLUDE=SCHEMA_EXPORT/TABLE schemas=IIMS_PRD
crrVar=3
crrEnv=PERFDBENV
flag=1
chmod 777 IIMSPRD_$timestamp.dmp
...
echo -e "\033[1;32mSelect environment From Where You Want To Export Table Dump: 
		...
		#press 3 for PERF-ENV Export Table Dump
...
if [ $variable2 -eq 1 -o $variable2 -eq 2 -o $variable2 -eq 3 -o $variable2 -eq 4 -o $variable2 -eq 5 ]
then
...
echo -e "\033[1;32mChoose Action You Want To Perform: 
		...
		#press 3 for Import in PERF-ENV
...
then
	...
	then
expdp system/TCS123@STCM directory=DUMP_bkup dumpfile=IIMSPRD_Bck_ST_$timestamp.dmp logfile=IIMSPRD_Bck_Export_$timestamp.log INCLUDE=SCHEMA_EXPORT/TABLE schemas=IIMS_PRD
echo "PRD Backup of STCM has been taken as IIMSPRD_Bck_ST_"$timestamp".dmp"
impdp system/TCS123@STCM directory=DUMP_BKUP schemas=IIMS_PRD TABLE_EXISTS_ACTION=REPLACE EXCLUDE=STATISTICS dumpfile=IIMSPRD_$timestamp.dmp LOGFILE=IIMSPRD_Import_$timestamp.log
	fi
	if [ $flag -eq 2 -o $flag -eq 4 ] 
	then
expdp system/TCS123@STCM directory=DUMP_bkup dumpfile=Table_Bck_ST_$timestamp.dmp logfile=Table_Export_$timestamp.log TABLES=$tableVar
echo "Table Backup of STCM has been taken as Table_Bck_ST_"$timestamp".dmp"
impdp system/TCS123@STCM directory=DUMP_BKUP TABLE_EXISTS_ACTION=REPLACE EXCLUDE=STATISTICS dumpfile=Table_$timestamp.dmp LOGFILE=Table_Import_$timestamp.log TABLES=$tableVar
	fi
elif [ $variable3 -eq 2 ]
then
	...
	if [ $flag -eq 1 -o $flag -eq 2 ] 
	then
impdp system/IIMS@devcm directory=DUMP_BKUP schemas=IIMS_PRD TABLE_EXISTS_ACTION=REPLACE EXCLUDE=STATISTICS dumpfile=IIMSPRD_$timestamp.dmp LOGFILE=IIMSPRD_Import_$timestamp.log
	fi
	if [ $flag -eq 2 -o $flag -eq 4 ] 
	then
impdp system/IIMSTCS@devcm directory=DUMP_BKUP TABLE_EXISTS_ACTION=REPLACE EXCLUDE=STATISTICS dumpfile=Table_$timestamp.dmp LOGFILE=Table_Import_$timestamp.log TABLES=$tableVar
	fi
elif [ $variable3 -eq 3 ]
then
	...
	if [ $flag -eq 1 -o $flag -eq 2 ] 
	then
impdp system/IIMS@PERFDBENV directory=DUMP_BKUP schemas=IIMS_PRD TABLE_EXISTS_ACTION=REPLACE EXCLUDE=STATISTICS dumpfile=IIMSPRD_$timestamp.dmp LOGFILE=IIMSPRD_Import_$timestamp.log
	fi
	if [ $flag -eq 2 -o $flag -eq 4 ] 
	then
impdp system/IIMS@PERFDBENV directory=DUMP_BKUP TABLE_EXISTS_ACTION=REPLACE EXCLUDE=STATISTICS dumpfile=Table_$timestamp.dmp LOGFILE=Table_Import_$timestamp.log TABLES=$tableVar
	fi
elif [ $variable3 -eq 4 ]
then
	...
	if [ $flag -eq 1 -o $flag -eq 2 ] 
	then
impdp system/IIMS@INTCM directory=DUMP_BKUP schemas=IIMS_PRD TABLE_EXISTS_ACTION=REPLACE EXCLUDE=STATISTICS dumpfile=IIMSPRD_$timestamp.dmp LOGFILE=IIMSPRD_Import_$timestamp.log
	fi
	if [ $flag -eq 2 -o $flag -eq 4 ] 
	then
impdp system/IIMSTCS@INTCM directory=DUMP_BKUP TABLE_EXISTS_ACTION=REPLACE EXCLUDE=STATISTICS dumpfile=Table_$timestamp.dmp LOGFILE=Table_Import_$timestamp.log TABLES=$tableVar
	fi
else
